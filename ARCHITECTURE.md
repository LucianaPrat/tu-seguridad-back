...
- **The breaker does not count a throttle.** The upstream is IP-throttled and documents neither the limit nor a `Retry-After`. A `429` therefore gets its own code, `UPSTREAM_THROTTLED` (503 outbound — nothing this caller did was rate limited), is excluded from the breaker through opossum's `errorFilter`, and parks detection in the client for `Retry-After` or five seconds. Without that filter a burst of `429`s trips a 50%-error breaker and every camera's detection short-circuits for the whole `resetTimeout`, reported as an upstream outage — a far longer outage than the throttle, caused by our own accounting. Measured behaviour that motivated it: [`plans/05.detection-quality.md`](plans/05.detection-quality.md) §2.5.

- **The detector's own answer is a metric.** `pipeline_persons_detected_total{cameraId,outcome}` splits a poll's detection into `persons`, `empty` (the upstream reported nobody) and `filtered` (it reported somebody below this camera's threshold). A poll counted `success` only ever said the pipeline ran; it could not distinguish an empty scene from an upstream that stopped finding people, which is exactly the question a "why did nothing alert" report asks. The gap between `persons` and the alerts actually raised is the unconfirmed-sighting rate.
...
- **A push transport does not retire the poll.** With `DVR_EVENTS_ENABLED` the recorder publishes motion over ISAPI `alertStream` and `DvrEventListener` turns each notification into the poll that already exists — but the poll is retuned from a fifteen-second cadence to a five-minute watchdog rather than switched off. The push has no acknowledgement, no replay, and a socket that dies silently: a recorder rebooting at 03:00 stops publishing and says nothing, and a security product whose only detection path can fail closed with no signal is not one. The watchdog bounds that failure at five minutes on every camera. It is also why the reconnect backoff is a module constant and not a knob — reconnect timing can only delay an alert, never lose one — while the idle window is a variable, because it tracks a heartbeat interval the recorder chooses. `DVR_EVENTS_ENABLED=false` restores poll-only behaviour, and `POST /dvr/event-linkage` is a separate admin call rather than a step inside `PUT /dvr` because it writes configuration into security hardware and must never be a side effect of saving a password.
- **Graceful shutdown ordering.** `main.ts` calls `enableShutdownHooks()`. Ordered teardown — stop new poll ticks → abort open recorder event streams → disconnect WS clients cleanly → disconnect Prisma — via Nest lifecycle hooks (`onModuleDestroy` on `EventsGateway`, `PollingScheduler`, `DvrEventListener`, `PrismaService`), **not** custom signal handlers. Nest tears down in reverse init order → feature modules run before shared `DataModule`, the order we want (WS/scheduler before Prisma). In-flight polls allowed to finish; only new ticks stopped.
