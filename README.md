...
All 25 setup-plan tasks done — see [`plans/01.setup.tasks.md`](plans/01.setup.tasks.md) for task-by-task record (what built, how verified, any deviations). Second plan, [`02.infra-hardening`](plans/02.infra-hardening.md), in progress: dependency scanning, face-auth circuit breaker, Sentry error tracking, deeper health checks + graceful shutdown, OpenAPI contract artifact, Prometheus metrics all **merged** (see [Observability, resilience & supply chain](#observability-resilience--supply-chain)); deploy pipeline and `sops` secrets still open — live status in [`plans/02.infra-hardening.tasks.md`](plans/02.infra-hardening.tasks.md).
...
Everything prefixed `/api/v1` (global prefix `api` + URI versioning) except `/docs*`, `/health/*` and `/metrics` — version-neutral, unprefixed. Bearer JWT is the default; `@Public()` routes are the exception, and they are exactly the ones that cannot have a session yet or are called by something that is not a browser: login, register, refresh, logout, the password-reset and magic-link pairs, face login, invitation acceptance, the provider acknowledgement webhook, health, docs and the scaffold root route.
...
4. `OccupancyEngine.evaluate` (`src/modules/pipeline/occupancy.engine.ts`) tests each anchor against the monitored areas — the outline when the operator drew one, its bounding box otherwise; one implicit full-frame area in `monitorMode = full`, the camera's `monitor_zones` in `partial` — and applies **hysteresis**: an area is entered once `ENTER_HITS_REQUIRED` of the last `ENTER_WINDOW_POLLS` polls put ≥1 anchor inside it, and exited after `EXIT_CONSECUTIVE_POLLS` consecutive misses. A single extra poll (flicker) raises nothing, and a single *missed* poll no longer throws the sighting away — that is the whole reason entry is a sliding window instead of a consecutive run, since the upstream detector drops frames of a subject that never moved ([`plans/05.detection-quality.md`](plans/05.detection-quality.md)). Exit is still a consecutive run, and one frame that sees the person again resets the miss count to zero.
...
The cameras that *are* due are polled **`POLLING_CONCURRENCY` at a time** (default `4`), not one after another. Awaiting each in turn made a cycle cost the sum of every camera's poll, so the cadences above were a ceiling nothing could reach: eight cameras at two seconds each put every one of them on a sixteen-second cycle no matter what its frame showed, and one unreachable recorder burning its timeout delayed every camera behind it. The bound is the point — each poll is an ISAPI capture on the space's recorder plus a detection POST upstream, so an unbounded `Promise.all` would fire the whole estate at both at once. `1` restores the serial tick.
...
- **Row count is bounded, and so is write volume now.** A frame that raises an alert gets its own immutable evidence row. Everything else shares one **live row per camera** (`snapshots.is_live`), overwritten in place: the poll refreshes it, and `POST /cameras/:id/snapshots` rewrites the same row instead of adding one. So a quiet camera costs one row forever, not one per poll. Bounded row count was never bounded write volume, though: every tick rewrote the whole JPEG, which at eight cameras on the passive rung is ~46k `MEDIUMBLOB` updates a day. `SNAPSHOT_LIVE_WRITE_SECONDS` (default `300`) is the floor between two live writes for one camera on the poll path — the same eight cameras cost ~2.3k writes a day instead. Two consequences, both deliberate: the stored frame can be up to one window older than `cameras.last_snapshot_at`, which is the recorder's own freshness and still moves every poll; and between live writes an alert's evidence row can hold the newest `captured_at` for that camera, so `latestSnapshotUrl` resolves to the annotated evidence frame rather than the plain live one. `0` restores a write on every poll. The manual capture route is never throttled — the operator clicked, the frame is fresh.
...
| Tracing | OpenTelemetry SDK + a collector sidecar | `src/observability/tracing.ts`, `tracing.helpers.ts`, [`ops/otel-collector/`](ops/otel-collector/README.md) | Opt-in via `OTEL_ENABLED`. Loaded **before** `ConfigModule` (reads `dotenv` itself). `withSpan(name, attrs, fn)` wraps spans; no-op when disabled. Two spans are opened by hand — `poll.camera` per camera poll and `face-auth.detect` — and the http auto-instrumentation hangs the rest under them: both DVR requests per ISAPI capture (the digest challenge and the authorized retry), the detection POST, and the Prisma queries. So one poll is one trace, and the cycle time and its split per stage are read off it. OTLP goes to the collector on `127.0.0.1:4318`, which forwards to Grafana Cloud; nothing in this process talks to Grafana. |
...
> **In flight** (open PRs, tracked in [`plans/02.infra-hardening.md`](plans/02.infra-hardening.md), not yet on `develop`): SSH/PM2 deploy pipeline (T02), `sops`/`age` secrets (T06). Check [`plans/02.infra-hardening.tasks.md`](plans/02.infra-hardening.tasks.md) for live status before assuming any present. That plan's "`snapshotUrl` field encryption at rest" item is **closed by other means**: plan 03 removed the column, so no credential-bearing URL is persisted at all, and the field encryption it asked for now protects `dvrs.password_encrypted` instead.
...
A second PM2 process shares the host when tracing is on: the collector in [`ops/otel-collector/`](ops/otel-collector/README.md), started and restarted **by hand** with its own scripts. `deploy.yml` does not touch it, so a deploy never takes it down and a config change there never waits on a release.
...
| `POLLING_PASSIVE_SECONDS` | `15`, `300` once `DVR_EVENTS_ENABLED=true` | Poll cadence for a camera whose last frame held nobody, `1`–`3600`. Once event-driven capture is on, this rung is a watchdog rather than the primary path, so its own default moves to `300` — a flat change to `300` would regress any deployment that upgrades without turning events on, 20× less responsive with nothing replacing the poll. `POLLING_ACTIVE_SECONDS` and `POLLING_DETECTION_SECONDS` are unchanged, and the process still ticks at the shortest of the three — `5` seconds. See [Poll cadence](#poll-cadence). |
...
| `POLLING_CONCURRENCY` | `4` | How many cameras one tick polls at a time, `1`–`64`. The tick used to await each camera in turn, so a cycle cost the sum of every camera's poll and the cadences above became unreachable long before the recorder was busy. Bounded rather than unbounded because each poll is one ISAPI capture on the space's recorder plus one detection POST upstream. `1` restores the serial tick. See [Poll cadence](#poll-cadence). |
| `DVR_EVENTS_ENABLED` | `false` | Master switch for event-driven capture off the recorder's motion-detection push connection. Off — same posture as `POLLING_ENABLED` and `MEDIAMTX_ENABLED` — the poll above alone drives captures exactly as before; on, it opens and holds a socket to the recorder, so a developer who pulls this branch must not find the process talking to an appliance unasked. |
| `DVR_EVENTS_DEBOUNCE_SECONDS` | `5` | Floor between two event-driven captures of the same camera, `1`–`3600`. The recorder repeats `VMD active` for as long as motion lasts and never sends an "inactive" — one person walking past produced 5 pulses in under 10 seconds — and the scheduler's in-flight guard refuses only overlap, not repetition. Default matches `POLLING_DETECTION_SECONDS`: a plain default, deliberately not a ref to it, so lowering the poll cannot silently loosen this too. `1`, not `0`, because `0` against the IP-throttled detector is the `429` storm this repo has already had ([`plans/05.detection-quality.md`](plans/05.detection-quality.md) §2.5). |
| `DVR_EVENTS_IDLE_SECONDS` | `30` | Silence past which the event connection is considered dead, `15`–`3600` — the recorder's idle heartbeat lands roughly every 9.5 seconds, so the default is three missed beats. The floor sits above one beat, so no accepted value can reconnect between two that were never actually missed; it is the only liveness signal a half-open socket gives. |
...
| `ENTER_HITS_REQUIRED` / `ENTER_WINDOW_POLLS` | `2` / `4` | Entry confirmation: an area is entered once `ENTER_HITS_REQUIRED` of the last `ENTER_WINDOW_POLLS` frames put a person inside it. A window rather than a run of consecutive hits because the upstream detector drops single frames of a subject that never left — measured at one detection in 33 frames of an IR night scene, which makes two consecutive hits unreachable. Equal values reproduce the old consecutive rule exactly. Boot refuses hits above the window. See [`plans/05.detection-quality.md`](plans/05.detection-quality.md). |
| `EXIT_CONSECUTIVE_POLLS` | `3` | Frames with nobody inside before an exit is confirmed. Stays consecutive on purpose: a premature exit only re-arms the area, while a lingering one suppresses the next real alert. |
| `SNAPSHOT_KEEP_RAW` | `false` | Store the untouched capture alongside the annotated evidence frame. Off by default — it doubles the blob volume of the highest-volume table, and the annotated frame is what the email and dashboard read. On when the detector's own recall is under audit: re-running history against the upstream is only exact on the pixels it answered. Never applies to live thumbnails, and the retention sweep prunes both rows. |
| `SNAPSHOT_KEEP_MISSES` | `false` | Keep the frame of an event-triggered poll the detector answered *nobody* — the recall ledger. The recorder classified the motion as human or vehicle and the upstream still found no one, so the frame is a labelled miss, and today it is discarded because only an alert persists one. Off by default: it writes a blob on a path that raises no alert. Rows carry `reason = ledger_miss`, are exported by `scripts/export-ledger.ts` for `scripts/try-detect.ts` to re-score, and are swept on the ordinary `RETENTION_SNAPSHOT_DAYS` window. A frame the camera's own threshold dropped is *not* kept — that is our filter refusing a sighting, a different problem. See [`plans/07.playback-backfill.md`](plans/07.playback-backfill.md). |
| `ALERT_COOLDOWN_SECONDS` | `0` (off) | Seconds before the same camera, zone and alert type can raise a second alert, `0`–`3600`. The hysteresis collapses a flickering detection into one cycle; this collapses a person who leaves the frame for a poll and comes back. A different alert type on the same zone is never suppressed. Suppressed candidates are counted on `pipeline_alerts_suppressed_total`, never silent. |
| `THROTTLE_TTL_SECONDS` / `THROTTLE_LIMIT` | `1` / `10` | Global inbound rate limit, per client IP, in every environment. Credential routes (login, register, password reset, magic link, invitation acceptance, face login) carry a tighter limit of their own — ten a minute — and `POST /events/acknowledgements` thirty a minute; both are constants in `src/cross/common/constants.ts`, not variables, because they are product rules. `POST /streaming/authorize` is exempt: the media server calls it once per segment. |
| `SWAGGER_ENABLED` | `true`, `false` in production | Serves the interactive docs at `/docs`. Off in production by default: the page is a full inventory of the routes, their bodies and their failure codes. `npm run openapi:export` builds the contract directly and is unaffected either way. |
...
| `METRICS_TOKEN` | — | Shared secret `GET /metrics` demands in the `X-Metrics-Token` header. **Required in production** — nothing else fronts that route — and unset in dev, which leaves it open for a local scraper. |
...
| [`ops/otel-collector/`](ops/otel-collector/README.md) | The trace collector sidecar: modes, scripts, PM2, why it is its own instance and not face-api's. Deployed by hand, not by `deploy.yml`. |
