...
| `DVR_EVENTS_IDLE_SECONDS` | `30` | Silence past which the event connection is considered dead, `5`–`3600` — the recorder's idle heartbeat lands roughly every 9.5 seconds, so this is three missed beats. The floor sits above one beat so it cannot be set low enough to reconnect between two heartbeats that were never actually missed; it is the only liveness signal a half-open socket gives. |
